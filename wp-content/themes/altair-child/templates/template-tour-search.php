    ...
    <form id="tour_search_form" name="tour_search_form" method="get" action="<?php echo get_the_permalink($id); ?>">
        <div class="tour_search_wrapper">
            <div class="one_fourth">
                <label for="keyword"><?php echo _e( 'Destinație', THEMEDOMAIN ); ?></label>
                <input id="keyword" name="keyword" type="text" placeholder="<?php echo _e( 'Oraș, regiune, cuvinte cheie', THEMEDOMAIN ); ?>" <?php if(isset($_GET['keyword'])) { ?>value="<?php echo $_GET['keyword']; ?>"<?php } ?>/>
            </div>
            <div class="one_fourth">
                <label for="start_date"><?php echo _e( 'Data', THEMEDOMAIN ); ?></label>
                <div class="start_date_input">
                    <input id="start_date" name="start_date" type="text" placeholder="<?php echo _e( 'Plecare', THEMEDOMAIN ); ?>" <?php if(isset($_GET['start_date'])) { ?>value="<?php echo $_GET['start_date']; ?>"<?php } ?>/>
                ...
                <div class="end_date_input">
                    <input id="end_date" name="end_date" type="text" placeholder="<?php echo _e( 'Sosire', THEMEDOMAIN ); ?>" <?php if(isset($_GET['end_date'])) { ?>value="<?php echo $_GET['end_date']; ?>"<?php } ?>/>
                    <input id="end_date_raw" name="end_date_raw" type="hidden" <?php if(isset($_GET['end_date_raw'])) { ?>value="<?php echo $_GET['end_date_raw']; ?>"<?php } ?>/>
                    <i class="fa fa-calendar"></i>
                </div>
            </div>
            <div class="one_fourth">
                <label for="budget"><?php echo _e( 'Buget maxim', THEMEDOMAIN ); ?></label>
                <input id="budget" name="budget" type="text" placeholder="<?php echo _e( '1000', THEMEDOMAIN ); ?>" <?php if(isset($_GET['budget'])) { ?>value="<?php echo $_GET['budget']; ?>"<?php } ?>/>
            </div>
            <div class="one_fourth last">
                <input id="tour_search_btn" type="submit" value="<?php echo _e( 'Caută', THEMEDOMAIN ); ?>"/>
            </div>
        </div>
    </form>
    <?php
